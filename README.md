...
The repository continues to describe portable application intent. The selected BaseHarbor **target** describes where BaseHarbor operates: runtime provider, runtime access reference and target scope. The **environment** remains application/deployment policy and is not inferred from the runtime target.
...
The active target is intended to be shell-local, similar to a Python virtual environment, so multiple terminals can safely target different targets at the same time. Optional prompt integration can keep the target visible before a command is entered, for example:
...
The concrete v0.4.15 target/deployment-registry implementation is tracked in BaseHarbor issue #408. Kubernetes/OpenShift runtime implementations remain later work; the point of the v0.4.15 foundation is that they can be added without changing the portable application contract.
