...
container="$(docker ps -q --filter "label=com.docker.compose.project=$project" --filter 'label=com.docker.compose.service=demo-app' | head -1)"
test -n "$container"
...
postgres="$(docker ps -q --filter label=com.docker.compose.project=baseharbor-baseharbor-demo-dev --filter label=com.docker.compose.service=postgres | head -1 || true)"
if [ -n "$postgres" ]; then
